...
def sentence1() -> Expr:
    ...
    st = A | B
    nd = ~A % (~B | C)
    rd = logic.disjoin(~A,~B,C)
    return logic.conjoin(st,nd,rd)
...
def sentence2() -> Expr:
    ...
    rd = ~(B & ~C) >> A
    fourth = ~D >> C
    return logic.conjoin(st,nd,rd,fourth)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    cant_both = ~(b & c)
    born = c
    return logic.conjoin(alive,cant_both,born) 
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    current_list = []
    for i in comb:
        current_list.append(disjoin(~i[0], ~i[1])) 
    
    return conjoin(current_list)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for i, j in all_coords:
        pacphysics_sentences.append(PropSymbolExpr(wall_str, i, j) >> ~PropSymbolExpr(pacman_str, i, j, time=t))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    Expr1 = conjoin(KB)
    Expr2 = PropSymbolExpr(pacman_str, x1, y1, time = 1)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        if findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time = t)) != False:
            return extractActionSequence(findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time = t)), actions)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for i, j in non_outer_wall_coords:
            if entails(conjoin(KB), PropSymbolExpr(wall_str, i, j)):
                KB.append(PropSymbolExpr(wall_str, i, j))
                known_map[i][j] = 1
            elif entails(conjoin(KB), ~PropSymbolExpr(wall_str, i, j)):
                KB.append(~PropSymbolExpr(wall_str, i, j))
                known_map[i][j] = 0
...
def slam(problem, agent) -> Generator:
    ...
    #We don't have a wall in the initial position
    KB.append(~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
    known_map[pac_x_0][pac_y_0] = 0
